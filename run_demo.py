...
def main():
    print("=" * 62)
    print("  RoadLens AI — demo run (v2.0 with Predictive Engine)")
    print("=" * 62)
    ...
    if len(tickets) >= 3:
        ...
        recurrence = predictive.check_recurrence(
            ticket_id=ticket["id"],
            lat=ticket["lat"] + 0.00001,
            lon=ticket["lon"] + 0.00001,
            defect_type=ticket["defect_type"],
        )
        if recurrence:
            store.record_recurrence(ticket["id"])
            print(f"   Recurrence detected for {ticket['id']}!")
            print(f"   Recurrence count: {recurrence['recurrence_count']}")
            print(f"   Repair quality score: {recurrence['repair_quality_score']}")
            print(f"   Recommended action: {recurrence['recommended_action']}")
        else:
            print("   No recurrence detected (expected for first check)")
    ...
    with open(DASH_DATA, "w") as f:
        ...
        f.write("// without the API server running.\n")
        f.write("window.ROADLENS_DEMO = ")
        json.dump({"tickets": all_tickets, "stats": stats}, f, indent=1)
        f.write(";\n")
    ...
    crew_perf = predictive.get_crew_performance()
    if crew_perf:
        print("\nCrew Performance:")
        for c in crew_perf:
